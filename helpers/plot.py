...
def freq_histogram_col(df, title="Freq. Hist", binsize: int | float = 0.1, legend_dict: dict = None) -> None:
    # Create bins for the histogram
    bins = np.arange(df.spikes.min(), df.spikes.max(), binsize)

    # Calculate histogram for the event
    y_height, x_bins = np.histogram(df[df.color == 'red'].spikes, bins=bins)

    # Plot event bars
    plt.bar(x_bins[:-1], y_height, width=binsize, color='red')

    # Calculate histogram for pre-event
    y_height, x_bins = np.histogram(df[df.color == 'blue'].spikes, bins=bins)

    # Plot pre-event bars
    plt.bar(x_bins[:-1], y_height, width=binsize, color='blue')

    plt.title(f"{title}", fontweight='bold')
    plt.xlabel("Time (s)")
    plt.ylabel("Spikes / second", fontweight='bold')
    plt.show()


def freq_histogram(spikes, title="Freq. Hist", binsize: int | float = 0.1, legend_dict: dict = None) -> None:
    if not isinstance(spikes, pd.DataFrame):
    ...
    fig, ax = plt.subplots()
    ax.bar(x_bins[:-1], y_height, width=.1)
