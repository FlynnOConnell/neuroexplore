    ...
    def get_eating_stats(self):
        for file in self.files['rf']:
            for filename, data in file.items():
                ...
                self.means_df = concat_dataframe_to_dataframe(self.means_df, data.means)
                self.sems_df = concat_dataframe_to_dataframe(self.sems_df, data.sems)
