...
class Reader(object):
    ...
    def ReadNex5File(self, filePath: str) -> dict:
        ...
        if metaOffset > 0:
            ...
            if metaOffset < size:
                ...
                metaString = (
                    self.theFile.read(size - metaOffset).decode("utf-8").strip("\x00")
                )
    ...
    def _ReadData(self):
        for var in self.fileData["Variables"]:
            ...
            methods = [
                self._ReadTimestamps,
                self._ReadTimestamps,
                self._ReadIntervals,
                self._ReadWaveforms,
                self._ReadPopVectors,
    ...
    def _ToString(self, theBytes, discardAfterFirstZero=False):
        ...
        try:
            ...
            try:
                ...
                return theBytesBeforeZero.decode("utf-8", errors="replace").strip(
                    "\x00"
                )
    ...
    def _ReadTimestamps(self, var):
        ...
        var["Timestamps"] = self._ReadAndScaleValues(
            tsValueType, var["Header"]["Count"], self.tsFreq, True
        )
    ...
    def _ReadIntervals(self, var):
        ...
        if var["Header"]["TsDataType"] == 1:
            tsValueType = DataFormat.INT64
        intStarts = self._ReadAndScaleValues(
            tsValueType, var["Header"]["Count"], self.tsFreq, True
        )
        intEnds = self._ReadAndScaleValues(
            tsValueType, var["Header"]["Count"], self.tsFreq, True
        )
    ...
    def _ReadWaveforms(self, var):
        ...
        wf = self._ReadAndScaleValues(
            wfValueType, var["Header"]["Count"] * var["Header"]["NPointsWave"], coeff
        )
        if self.useNumpy:
            import numpy as np

            var["WaveformValues"] = wf.reshape(
                var["Header"]["Count"], var["Header"]["NPointsWave"]
            )
            if woffset != 0:
                var["WaveformValues"] = var["WaveformValues"] + woffset
        else:
            if woffset != 0:
                var["WaveformValues"] = list(
                    self._Chunks(
                        [x + woffset for x in wf], var["Header"]["NPointsWave"]
                    )
                )
            else:
                var["WaveformValues"] = list(
                    self._Chunks(wf, var["Header"]["NPointsWave"])
                )
    ...
    def _ReadPopVectors(self, var):
        var["Weights"] = self._ReadAndScaleValues(
            DataFormat.FLOAT64, var["Header"]["Count"]
        )
    ...
    def _ReadContinuous(self, var):
        ...
        if var["Header"]["TsDataType"] == 1:
            tsValueType = DataFormat.INT64
        var["FragmentTimestamps"] = self._ReadAndScaleValues(
            tsValueType, var["Header"]["Count"], self.tsFreq, True
        )
        indexValueType = DataFormat.UINT32
        if var["Header"]["ContFragIndexType"] == 1:
            indexValueType = DataFormat.UINT64
        var["FragmentIndexes"] = self._ReadAndScaleValues(
            indexValueType, var["Header"]["Count"]
        )
        var["FragmentCounts"] = []
        ...
        if var["Header"]["ContDataType"] == 1:
            ...
            coeff = 1.0
            woffset = 0.0
        var["ContinuousValues"] = self._ReadAndScaleValues(
            contValueType, var["Header"]["NPointsWave"], coeff
        )
        if woffset != 0:
    ...
    def _ReadMarker(self, var):
        ...
        for field in range(var["Header"]["NMarkers"]):
            ...
            if var["Header"]["MarkerDataType"] == 0:
                ...
                field["Markers"] = [
                    self._ToString(self.theFile.read(length), True)
                    for m in range(var["Header"]["Count"])
                ]
                if isNumeric:
                ...
                field["Markers"] = self._ReadAndScaleValues(
                    DataFormat.UINT32, var["Header"]["Count"]
                )
                var["Fields"].append(field)
...
class NexWriter(object):
    ...
    def __init__(self, timestampFrequency, useNumpy=False):
        ...
        self.nex5fileHeaderKeysToWrite = [
            "MagicNumber",
            "NexFileVersion",
            "Comment",
            "Frequency",
    ...
    def AddContVarWithSingleFragment(
        self, name, timestampOfFirstDataPoint, SamplingRate, values
    ):
        """
    ...
    def AddContVarWithMultipleFragments(
        self, name, timestamps, SamplingRate, fragmentValues
    ):
        """
    ...
    def AddMarker(self, name, timestamps, fieldNames, markerFields):
        ...
        for x in markerFields:
            if len(x) != len(timestamps):
                raise ValueError(
                    "different number of timestamps and field values in a single field"
                )
        vhValues = [
    ...
    def WriteNexFile(self, filePath):
        ...
        self.fileData["FileHeader"]["BegTicks"] = int(
            round(self.fileData["FileHeader"]["Beg"] * self.tsFreq)
        )
        ...
        for i in range(len(self.fileHeaderKeysToWrite)):
            self._WriteField(
                fileHeaderFormat[i],
                self.fileData["FileHeader"][self.fileHeaderKeysToWrite[i]],
            )

        varHeaderFormat = (
            "<i <i 64s <i <i <i <i <i <i <d <d <d <d <i <i <i <d <d 52s".split()
        )
        for v in self.fileData["Variables"]:
    ...
    def WriteNex5File(self, filePath, saveContValuesAsFloats=0):
        ...
        for i in range(len(self.nex5fileHeaderKeysToWrite)):
            self._WriteField(
                fileHeaderFormat[i],
                self.fileData["FileHeader"][self.nex5fileHeaderKeysToWrite[i]],
            )

        varHeaderFormat = (
            "<i <i 64s <Q <Q <i <i <d 32s <d <d <Q <d <i <i <i <i 60s".split()
        )
        for v in self.fileData["Variables"]:
            for i in range(len(self.nex5VarHeaderKeys)):
                self._WriteField(
                    varHeaderFormat[i], v["Header"][self.nex5VarHeaderKeys[i]]
                )
        ...
        for v in self.fileData["Variables"]:
            ...
            if (
                v["Header"]["Type"] == NexFileVarType.NEURON
                or v["Header"]["Type"] == NexFileVarType.WAVEFORM
            ):
    ...
    def _VarNumDataBytes(self, var):
        ...
        if varType == NexFileVarType.NEURON or varType == NexFileVarType.EVENT:
            ...
            return (
                self._BytesInTimestamp(var) * len(var["Timestamps"])
                + self._BytesInContValue(var)
                * len(var["Timestamps"])
                * var["Header"]["NPointsWave"]
            )
            ...
            return (self._BytesInTimestamp(var) + 4) * len(
                var["Timestamps"]
            ) + self._BytesInContValue(var) * len(var["ContinuousValues"])
            ...
            return self._BytesInTimestamp(var) * nts + nm * (
                64 + nts * singleMarkerLength
            )
    ...
    def _VarMaxTimestampNumpy(self, var):
        ...
        varType = var["Header"]["Type"]
        if (
            varType == NexFileVarType.NEURON
            or varType == NexFileVarType.EVENT
            or varType == NexFileVarType.MARKER
        ):
            ...
            if len(var["Timestamps"]) == 0:
                return 0
            return (
                self._MaxOfNumpyArrayOrZero(var["Timestamps"])
                + (var["Header"]["NPointsWave"] - 1) / var["Header"]["SamplingRate"]
            )
        elif varType == NexFileVarType.POPULATION_VECTOR:
            return 0
        elif varType == NexFileVarType.CONTINUOUS:
            ...
            return (
                var["Timestamps"][-1]
                + (var["FragmentCounts"][-1] - 1) / var["Header"]["SamplingRate"]
            )
    ...
    def _VarWriteWaveformsNumpy(self, var):
        ...
        if self._BytesInContValue(var) == 2:
            np.round(var["WaveformValues"] / var["Header"]["ADtoMV"]).astype(
                np.int16
            ).tofile(self.theFile)
        else:
    ...
    def _VarWriteContinuousValuesNumpy(self, var):
        ...
        if self._BytesInContValue(var) == 2:
            np.round(var["ContinuousValues"] / var["Header"]["ADtoMV"]).astype(
                np.int16
            ).tofile(self.theFile)
        else:
    ...
    def _WriteContinuousVarData(self, var):
        ...
        if self.useNumpy:
            ...
            if self._BytesInContValue(var) == 2:
                contValues = [
                    int(x / var["Header"]["ADtoMV"]) for x in var["ContinuousValues"]
                ]
            ...
            for x in field:
                if not isinstance(x, numbers.Number):
                    ...
                    if not isinstance(x, str):
                        raise ValueError(
                            "marker values should be either numbers or strings"
                        )
